...
def main() -> None:
    ...
    hourly_window = Window.partitionBy(
        "Page_id", date_format("Timestamp", "yyyy-MM-dd HH")
    ).orderBy("Timestamp")
    daily_window = Window.partitionBy(
        "Page_id", date_format("Timestamp", "yyyy-MM-dd")
    ).orderBy("Timestamp")
    ...
    hourly_stats = (
        ...
        .withColumn("Distinct_Users", size(collect_set("User_id").over(hourly_window)))
        .groupBy("Page_id")
        .agg(
    ...
    daily_stats = (
        ...
        .withColumn("Distinct_Users", size(collect_set("User_id").over(daily_window)))
        .groupBy("Page_id")
        .agg(
